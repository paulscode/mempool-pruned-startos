...
The Bitcoin Knots (RDTS) Companion has been removed from Select Node. It no longer has a chain of its own: the RDTS rules now take effect at the BLAKE2b fork rather than activating separately, so that option and Bitcoin Knots BLAKE2b were two names for the same destination. If your explorer was pointed at it, it now points at Bitcoin Knots BLAKE2b.
